...
class databaseconnection:
    ...
    def insert_initial_transform_log(self, batch_date, country_id, initial_status):
        query = f"""
            ...
            VALUES (%s, %s)
            """
        self.cursor.execute(query, (batch_date, country_id, initial_status))
        ...
        try:
            ...
            if row:
                ...
                update_query = f"""
                    ...
                    SET processed_directory_name = %s, processed_directory_name = %s, row_count = %s, status = %s
                    WHERE id = %s;
                    """
    ...
    def insert_weather_data(self, country_id, date, weather_code, mean_temperature, mean_surface_pressure, precipitation_sum, relative_humidity, wind_speed):
        query = f"""
            INSERT INTO transform.weather_data (country_id, date, weather_code, mean_temperature, mean_surface_pressure, precipitation_sum, relative_humidity, wind_speed)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """
        self.cursor.execute(query, (country_id, date, weather_code, mean_temperature, mean_surface_pressure, precipitation_sum, relative_humidity, wind_speed))
        self.connection.commit()
        print('Weather data record has been added!')

    def insert_covid_data(self, country_id, date, confirmed_cases, deaths, recovered):
        query = f"""
            INSERT INTO transform.covid_data (country_id, date, confirmed_cases, deaths, recovered)
            VALUES (%s, %s, %s, %s, %s)
            """
        self.cursor.execute(query, (country_id, date, confirmed_cases, deaths, recovered))
        self.connection.commit()
